...
export default class GroupSettingsTab extends Component {
  ...
  render () {
    ...
    const { edits, changed } = this.state
    const {
      avatarUrl, bannerUrl, description, location, moderatorDescriptor, moderatorDescriptorPlural, name, settings
    } = edits
    ...
    return <div styleName='general.groupSettings'>
      ...
      <SettingsControl
        label='Word to describe group Moderator'
        onChange={this.updateSetting('moderatorDescriptor')}
        value={moderatorDescriptor}
      />

      <SettingsControl
        label='Plural word to describe group Moderators'
